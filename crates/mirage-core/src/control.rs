...
pub enum PrecisionTier { Q2, Q4, Q6, FP8, FP16 }

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum ChainPhase { NotReasoning, Early, Middle, Ending }

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum DifficultyClass { Trivial, Normal, ReasoningCritical }

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum Device { Gpu, Cpu }

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub enum TokenKind { Punctuation, Formatting, CommonWord, Content, ReasoningMarker }
...
impl HeadMask {
    pub fn all(n_heads: u16) -> Self {
        ...
        HeadMask(if n_heads == 64 { u64::MAX } else { (1u64 << n_heads) - 1 })
    }
    pub fn count(&self) -> u32 { self.0.count_ones() }
    pub fn is_set(&self, head: u16) -> bool { self.0 & (1u64 << head) != 0 }
}
