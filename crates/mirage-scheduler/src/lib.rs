...
impl FallbackController {
    ...
    pub fn baseline_reachable(&self) -> bool {
        true
    }
}
...
impl<P: CognitiveProfiler, B: BranchPredictor> Scheduler<P, B> {
    ...
    pub fn step<E: ExecutionBackend>(
        ...
        let tier = if fallback.level() >= 1 {
            PrecisionTier::Q4
        } else {
            decision.precision_per_group[0]
        };
        let depth = if fallback.level() >= 3 {
            shape.n_layers
        } else {
            decision.target_depth
        };

        let predicted_flat: Vec<u16> = prediction
        ...
        let ops = (0..depth.min(shape.n_layers))
            .map(|l| LayerOp {
                layer_idx: l,
                precision: tier,
                head_mask: HeadMask::all(shape.n_heads),
                experts: ExpertBinding::Resident(predicted_flat.clone()),
                device: Device::Gpu,
            })
            .collect();
        ...
        let plan = ExecPlan {
            ...
            ops,
            exit_after: depth.min(shape.n_layers),
        ...
        let actual_flat: Vec<u16> = outcome
            .actual
            .experts_per_moe_layer
            .iter()
            .flatten()
        ...
        TokenTrace {
            ...
            token_id: tctx.token_id,
            position: tctx.position,
            chain_phase: decision.chain_phase,
            predicted_experts: predicted_flat,
            actual_experts: actual_flat,
            precision_plan: vec![tier],
            profiler_decision: decision,
            ...
            vram_usage_mb: t.vram_mb,
            pcie_bytes: t.pcie_bytes,
            kv_cache_bytes: t.kv_bytes,
            energy_mj: t.energy_mj,
            logits_entropy: tctx.recent_entropy,
            logit_margin: tctx.recent_logit_margin,
            fallback_level: fallback.level(),
            deterministic: true,
        }
    }
