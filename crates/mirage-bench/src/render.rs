...
pub fn to_markdown(reports: &[BenchReport]) -> String {
    ...
    writeln!(out, "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|").ok();
    for r in reports {
        let div = r.plan_divergence.as_ref()
            .map(|s| format!("{:.4} ({})", s.value, s.samples))
            .unwrap_or_else(|| "-".into());
        writeln!(
            out,
            "| {} | {} | {} | {:.1} | {} | {} | {:.3} | {:.1} | {} | {} | {} |",
            r.config_name, r.task_name, r.tokens, r.sim_tok_per_s,
            r.p50_total_us, r.p99_total_us, r.stall_fraction,
            r.mean_vram_mb, r.peak_vram_mb, r.fallback_level_max, div,
        ).ok();
    }
    out
}
