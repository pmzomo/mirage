...
impl NgramBranchPredictor {
    pub fn new(order: usize) -> Self {
        NgramBranchPredictor { table: HashMap::new(), order }
    }
...
impl BranchPredictor for NgramBranchPredictor {
    fn predict(&self, ctx: &RoutingContext) -> RoutingPrediction {
        let experts = self.table.get(&self.sig(&ctx.recent_experts))
            .cloned()
            .unwrap_or_default();
        RoutingPrediction { experts_per_moe_layer: experts, likely_exit_layer: 0 }
    }
    fn update(&mut self, _predicted: &RoutingPrediction, actual: &ActualRouting) {
        ...
        let flat: Vec<u16> = actual.experts_per_moe_layer.iter().flatten().copied().collect();
...
mod tests {
    ...
    fn learns_a_repeated_routing_pattern() {
        ...
        let actual = ActualRouting {
            experts_per_moe_layer: vec![vec![3, 7, 9, 1]], exit_layer: 1,
        };
        p.update(&RoutingPrediction::default(), &actual);
        let flat = vec![3u16, 7, 9, 1];
        let pred = p.predict(&RoutingContext { position: 1, recent_experts: flat });
    ...
    fn unknown_context_predicts_empty_not_panic() {
        ...
        let pred = p.predict(&RoutingContext { position: 0, recent_experts: vec![] });
